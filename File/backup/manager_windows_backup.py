...
MEMO_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "memos.json")
MESSAGE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "messages.json")
...
class AlarmService:
    ...
    def check_and_trigger_alarms(self):
        ...
        for alarm in self.alarms:
            if alarm.get("enabled", True) and alarm.get("time") == current_time:
                ...
                if last_triggered != current_time:
                    ...
                    # 如果是稍后提醒闹钟，记录待清理
                    if alarm.get("is_snooze", False):
                        triggered_snooze_alarms.append(alarm)
        ...
        if triggered_count > 0:
            ...
            if triggered_snooze_alarms:
                self._cleanup_triggered_snooze_alarms(triggered_snooze_alarms)
    
    def _cleanup_triggered_snooze_alarms(self, snooze_alarms):
        """清理已触发的稍后提醒闹钟"""
        alarms = load_alarms()
    ...
    def _cleanup_related_snooze_alarms(self, original_alarm):
        """清理与原始闹钟相关的所有稍后提醒实例"""
        alarms = load_alarms()
        original_count = len(alarms)
        
        # 获取原始闹钟的标签（不含" (稍后提醒)"后缀）
        original_label = original_alarm.get("label", "")
        # 如果当前是稍后提醒闹钟，提取原始标签
        if " (稍后提醒)" in original_label:
            original_label = original_label.replace(" (稍后提醒)", "")
        ...
        for alarm in alarms:
            ...
            # 如果是稍后提醒且标签匹配原始闹钟，则删除
            if alarm.get("is_snooze", False) and (
                alarm_label == original_label or 
                alarm_label.startswith(original_label + " (稍后提醒)")
            ):
                removed_count += 1
                print(f"🗑️ 删除稍后提醒: {alarm_label}")
            else:
                alarms_to_keep.append(alarm)
        ...
        if removed_count > 0:
            save_alarms(alarms_to_keep)
            print(f"🧹 已清理 {removed_count} 个相关的稍后提醒闹钟")
    ...
    def trigger_alarm(self, alarm):
        ...
        # 检查是否是稍后提醒闹钟
        is_snooze = alarm.get("is_snooze", False)
        ...
        add_message(
            ...
            extra_data={"alarm_label": label, "alarm_time": time_str, "is_snooze": is_snooze}
        )
    ...
    def snooze_alarm(self, alarm, notif_window, auto_trigger=False):
        ...
        snooze_alarm_data = {
            ...
            "label": f"{label} (稍后提醒)",
            "enabled": True,
            "sound_type": alarm.get("sound_type", "system"),
            "sound": alarm.get("sound", "default"),
            "is_snooze": True,  # 标记为稍后提醒
            "snooze_count": snooze_count,  # 记录重复次数
            "original_alarm_index": self.alarms.index(alarm) if alarm in self.alarms else -1  # 记录原始闹钟索引
        }
        
        # 如果是自定义文件，保存路径
        if alarm.get("sound_type") == "file" and "sound_path" in alarm:
            snooze_alarm_data["sound_path"] = alarm["sound_path"]
        
        # 添加到闹钟列表并保存
        alarms = load_alarms()
        alarms.append(snooze_alarm_data)
        save_alarms(alarms)
...
#   缺点：更新可能不及时
#
# 方法2：安装 nvidia-ml-py（仅NVIDIA显卡）
#   pip install nvidia-ml-py
#   优点：官方库，稳定可靠
#   缺点：仅支持NVIDIA显卡
#
# 如果未安装任何库，GPU显示将显示为 "🎮 GPU --"
# ============================================================
